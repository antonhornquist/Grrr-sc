GRScreenGrid : GRController {
	...
	*initClass {
		...
		keymaps = ( // TODO: add arrow keys and backspaceKeycode in keymap
			swedish_osx: #[
			...
			swedish_win7: #[
				...
				*/
				// TODO: backspace:
				/*
					backspaceKeycode=TODO
				*/
			]
		);
		this.setKeymap(\swedish_osx);
	}

	*setKeymap { |keymapName|
		keymap = keymaps[keymapName];
	}
...
Press buttons with mouse, or enable key control with ctrl-backspace and use keyboard. Use shift to hold buttons. Use caps lock to hold and toggle buttons. If the grid is larger than key control area (%x%) it is possible to switch between areas on the ScreenGrid using the arrow buttons. Also, as long as alt is key presses are redirected to next key control area.".format(this.class, keyControlAreaNumCols, keyControlAreaNumRows)
	...
	handleKeyControlEvent { |keycode, pressed|
		var keymapIndex = keymap.indexOf(keycode);
		keycode.debug; // TODO: this was added in ubuntu, where duplicate presses were noted. should be checked, fixed if needed, and this line deleted
		if (keymapIndex.notNil) {
			this.handleKeyControlKeymapEvent(keymapIndex, pressed);
		} {
	...
	handleKeyControlKeymapEvent { |keymapIndex, pressed|
		...
		if ((pressed.not and: this.holdModifier).not) { // Ignore key released if caps on or shift pressed
			...
			button = this.lookupScreenGridButton(keyControlAreaForThisKeyEvent, keymapIndex);
			button !? {
				if (modifierCapsIsOn) {
					button.toggleAction
				} {
	...
	lookupScreenGridButton { |areaIndex, keymapIndex|
		var point;
		point = keyControlAreaOrigins[areaIndex]+Point.new(keymapIndex mod: keyControlAreaNumCols, keymapIndex div: keyControlAreaNumCols);
	...
	prConfigureKeyboardActions {
		...
		topView.keyDownAction = { |view, char, modifiers, unicode, keycode|
			if (modifierCtrlIsPressed and: keycode == backspaceKeycode) {
